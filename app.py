...
st.set_page_config(page_title="Stock Market Analysis and Prediction", layout="wide")
...
df = yf.download(tickers=stock, period="8y", interval="1d", auto_adjust=False)
...
fig.update_layout(
    title=str(stock) + " Live Share Price:", yaxis_title="Stock Price (USD per Share)"
)
...
colors = np.where(df["Close"].values >= df["Open"].values, "green", "red")


fig1.add_trace(go.Bar(x=df.index, y=df["Volume"], marker_color=colors, name="Volume"))
...
fig2.add_trace(
    go.Bar(x=df.index, y=macd.macd_diff(), marker_color="orange", name="MACD Histogram")
)
...
tab1, tab2, tab3 = st.tabs(
    ["Stock Prices", "Volume", "Moving average convergence/divergence (MACD)"]
)
...
st.subheader("Stock average price prediction using LSTM for next 30 days from today")
...
df1 = df[["Close", "Open", "High", "Low"]].copy()
